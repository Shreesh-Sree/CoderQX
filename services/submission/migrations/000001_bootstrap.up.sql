-- Submission is the durable record of candidate work. Authorization projections
-- are private; the app role can only invoke the fail-closed access predicate.
...
CREATE SCHEMA app AUTHORIZATION aether_submission_owner;
CREATE SCHEMA authz AUTHORIZATION aether_submission_owner;
CREATE SCHEMA submission AUTHORIZATION aether_submission_owner;
CREATE SCHEMA extensions AUTHORIZATION aether_submission_owner;
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;
REVOKE ALL ON SCHEMA app, authz, submission, extensions FROM PUBLIC;
DO $extension_schema_access$
BEGIN
    IF has_schema_privilege('aether_submission_app', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_submission_authz_reader', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_submission_projection_worker', 'extensions', 'USAGE') THEN
        RAISE EXCEPTION 'runtime roles must not have USAGE on extensions schema';
    END IF;
END
$extension_schema_access$;
GRANT USAGE ON SCHEMA app, authz, submission TO aether_submission_app;
GRANT USAGE ON SCHEMA authz TO aether_submission_authz_reader, aether_submission_projection_worker;

ALTER DEFAULT PRIVILEGES FOR ROLE aether_submission_owner IN SCHEMA app
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_submission_owner IN SCHEMA authz
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_submission_owner IN SCHEMA submission
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
...
BEGIN
    ...
    IF raw_setting IS NULL OR raw_setting !~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
        RETURN NULL;
    END IF;
...
RETURNS bigint LANGUAGE plpgsql STABLE SET search_path = pg_catalog AS $$
DECLARE raw_setting text;
BEGIN
...
CREATE INDEX actor_tenant_authorizations_tenant_idx
    ON authz.actor_tenant_authorizations (tenant_id, actor_id) WHERE active;
...
-- The initial schema uses the final authorization-context contract. Context
-- rows are short lived and unlogged; an instance restart therefore denies
-- requests until a fresh central decision is obtained.
...
CREATE INDEX projection_inbox_messages_pending_idx
    ON authz.projection_inbox_messages (received_at)
...
BEGIN
    ...
    WHERE key_id = p_key_id
      AND audience = current_database()
       AND not_before <= context_now
       AND not_after > context_now
      AND retired_at IS NULL;

    canonical_envelope := format(
        'aether-authz-context-v2|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s',
        current_database(), p_key_id, p_capability_id, p_actor_id, p_tenant_id, p_authz_revision,
    ...
    IF signing_key IS NULL
       OR extensions.hmac(convert_to(canonical_envelope, 'UTF8'), signing_key, 'sha256') <> p_signature
    THEN
        RAISE EXCEPTION 'invalid signed authorization context' USING ERRCODE = '28000';
    END IF;
...
GRANT EXECUTE ON FUNCTION app.current_context_id() TO aether_submission_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows(uuid, text, text) TO aether_submission_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows_read(uuid, text, text, text) TO aether_submission_app;
...
GRANT SELECT ON authz.actor_tenant_authorizations TO aether_submission_authz_reader;
