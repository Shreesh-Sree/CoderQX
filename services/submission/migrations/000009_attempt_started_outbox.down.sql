...
REVOKE EXECUTE ON FUNCTION submission.prepare_attempt_started_outbox_event(uuid, uuid, uuid, uuid)
    FROM aether_submission_app;
DROP FUNCTION submission.prepare_attempt_started_outbox_event(uuid, uuid, uuid, uuid);
DROP INDEX app.outbox_events_attempt_started_once_idx;
