-- Publish a single analytics-safe fact only when submit_attempt inserted a new
-- submitted event. The candidate audit event remains append-only and has its own
...
CREATE UNIQUE INDEX outbox_events_attempt_submitted_once_idx
    ON app.outbox_events (tenant_id, aggregate_id)
    WHERE event_type = 'submission.attempt_submitted.v1';

CREATE FUNCTION submission.prepare_attempt_submitted_outbox_event(
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION 'attempt submitted outbox command is invalid' USING ERRCODE = '22023';
    END IF;

    -- A new command event is written by submit_attempt only on the first
    -- submission. An idempotency replay receives a newly generated event ID that
    ...
    WHERE attempt_row.tenant_id = p_tenant_id
      ...
      AND audit_event.event_type = 'submission.attempt.submitted.v1'
      AND audit_event.payload ->> 'attempt_id' = p_attempt_id::text
    FOR SHARE OF attempt_row;
    IF NOT FOUND THEN
        RETURN;
    END IF;
    IF attempt_record.submitted_at IS NULL THEN
        RAISE EXCEPTION 'submitted attempt has no submission timestamp' USING ERRCODE = '23514';
    END IF;
    ...
    IF EXISTS (
        ...
        WHERE outbox_event.tenant_id = p_tenant_id
          ...
          AND outbox_event.event_type = 'submission.attempt_submitted.v1'
    ) THEN
        RETURN;
    END IF;

    -- Extract evaluation request count from the audit event payload
    SELECT (audit_event.payload ->> 'evaluation_request_count')::integer
    INTO eval_request_count
    FROM submission.attempt_events AS audit_event
    WHERE audit_event.id = p_attempt_event_id;
    ...
    event_payload := jsonb_build_object(
        ...
        'evaluation_request_count', eval_request_count,
        'submitted_at', attempt_record.submitted_at
    );
    RETURN QUERY SELECT event_payload, attempt_record.submitted_at;
END
$function$;

REVOKE ALL ON FUNCTION submission.prepare_attempt_submitted_outbox_event(uuid, uuid, uuid, uuid)
    FROM PUBLIC;
GRANT EXECUTE ON FUNCTION submission.prepare_attempt_submitted_outbox_event(uuid, uuid, uuid, uuid)
