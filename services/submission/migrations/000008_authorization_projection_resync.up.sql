-- Recover safely when the authorization snapshot consumer is older than
-- stream retention: submissions stay deny-by-default until a complete,
-- manifest-verified targeted grant batch is durable locally.
...
BEGIN
    IF p_request_event_id IS NULL OR p_resync_id IS NULL
       ...
       OR p_target_service <> 'submission' OR p_reason !~ '^[a-z][a-z0-9._-]{0,63}$' THEN
    ...
    INSERT INTO app.outbox_events (
        ...
        'authz.grants_snapshot.resync_requested.submission.v1', 1, event_payload,
...
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_projection_resync_state,
    authz.authorization_projection_resync_items TO aether_submission_projection_worker;
