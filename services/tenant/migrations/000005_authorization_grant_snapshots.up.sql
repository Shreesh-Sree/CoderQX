...
WITH duplicate_platform_rows AS (
    SELECT ctid, row_number() OVER (
        PARTITION BY actor_id ORDER BY authz_revision DESC, updated_at DESC, ctid DESC
    ) AS position
    FROM authz.actor_tenant_authorizations WHERE grant_kind = 'platform'
)
DELETE FROM authz.actor_tenant_authorizations AS grant_row
USING duplicate_platform_rows AS duplicate_row
WHERE grant_row.ctid = duplicate_row.ctid AND duplicate_row.position > 1;
...
UPDATE authz.actor_tenant_authorizations SET grant_source_id = tenant_id
...
DECLARE normalized_tenant_id uuid; normalized_source_id uuid;
BEGIN
    ...
    normalized_source_id := CASE
        ...
        WHEN p_grant_kind = 'tenant' THEN p_tenant_id ELSE p_grant_source_id END;
    IF normalized_source_id IS NULL THEN RAISE EXCEPTION 'grant source is required for placement authorization'; END IF;
    ...
    ON CONFLICT (actor_id) DO UPDATE SET authz_revision = EXCLUDED.authz_revision, updated_at = clock_timestamp()
...
DECLARE grant_item jsonb; grant_kind text; grant_tenant_id uuid; grant_source_id uuid; grant_expires_at timestamptz;
BEGIN
    ...
    FOR grant_item IN SELECT value FROM jsonb_array_elements(p_grants) LOOP
        IF jsonb_typeof(grant_item) <> 'object' THEN RAISE EXCEPTION 'authorization grant must be an object'; END IF;
        ...
        END;
        IF (grant_kind = 'platform' AND grant_tenant_id = '00000000-0000-0000-0000-000000000000'::uuid AND grant_source_id = '00000000-0000-0000-0000-000000000000'::uuid)
           OR (grant_kind = 'tenant' AND grant_tenant_id IS NOT NULL AND grant_tenant_id = grant_source_id)
           OR (grant_kind = 'placement' AND grant_tenant_id IS NOT NULL AND grant_source_id IS NOT NULL) THEN CONTINUE; END IF;
    ...
    FOR grant_item IN SELECT value FROM jsonb_array_elements(p_grants) LOOP
        ...
        INSERT INTO authz.actor_tenant_authorizations (
            actor_id, tenant_id, authz_revision, is_authorized, grant_kind, grant_source_id, expires_at
        ) VALUES (p_actor_id, grant_tenant_id, p_authz_revision, true, grant_kind, grant_source_id, grant_expires_at);
    END LOOP;
    RETURN true;
END
$function$;
...
GRANT EXECUTE ON FUNCTION authz.has_platform_authorization_at(uuid, bigint) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.apply_authorization_snapshot(uuid, bigint, jsonb) TO aether_tenant_projection_worker;
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_snapshot_inbox_messages
    TO aether_tenant_projection_worker;
GRANT SELECT ON authz.principal_authorization_revisions TO aether_tenant_authz_reader;
