-- The migration login must be a member of aether_tenant_owner.  Deployment
...
CREATE SCHEMA IF NOT EXISTS extensions AUTHORIZATION aether_tenant_owner;
CREATE SCHEMA IF NOT EXISTS app AUTHORIZATION aether_tenant_owner;
CREATE SCHEMA IF NOT EXISTS authz AUTHORIZATION aether_tenant_owner;
CREATE SCHEMA IF NOT EXISTS tenant AUTHORIZATION aether_tenant_owner;
...
GRANT USAGE ON SCHEMA public TO aether_tenant_migrator;
REVOKE ALL ON SCHEMA extensions, app, authz, tenant FROM PUBLIC;
DO $extension_schema_access$
BEGIN
    IF has_schema_privilege('aether_tenant_app', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_tenant_authz_reader', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_tenant_projection_worker', 'extensions', 'USAGE') THEN
        RAISE EXCEPTION 'runtime roles must not have USAGE on extensions schema';
    END IF;
END
$extension_schema_access$;
GRANT USAGE ON SCHEMA app, authz, tenant TO aether_tenant_app;
GRANT USAGE ON SCHEMA authz TO aether_tenant_authz_reader, aether_tenant_projection_worker;
...
ALTER DEFAULT PRIVILEGES IN SCHEMA tenant REVOKE ALL ON TABLES FROM PUBLIC;
ALTER DEFAULT PRIVILEGES IN SCHEMA tenant REVOKE ALL ON SEQUENCES FROM PUBLIC;
ALTER DEFAULT PRIVILEGES IN SCHEMA tenant REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
...
GRANT EXECUTE ON FUNCTION app.current_context_id() TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.set_context(uuid, uuid, bigint, text, uuid, text, text, timestamptz, timestamptz, uuid, bytea) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_context_valid(uuid) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows(uuid, text, text) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_global_context_allows(text, text) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows_placement(uuid, text, text) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_context_actor_id() TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_context_has_platform_access() TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.current_context_valid_placement(uuid) TO aether_tenant_app;
GRANT EXECUTE ON FUNCTION authz.apply_tenant_authorization(uuid, uuid, bigint, boolean, text, uuid, timestamptz) TO aether_tenant_projection_worker;
GRANT SELECT ON authz.actor_tenant_authorizations TO aether_tenant_authz_reader;
