-- Close the recovery gap when the authorization snapshot durable is older
-- than the broker's retention window. RLS remains deny-by-default until a
-- targeted full grant batch and its immutable manifest have both arrived.
...
BEGIN
    IF p_request_event_id IS NULL OR p_resync_id IS NULL
       ...
       OR p_target_service <> 'tenant' OR p_reason !~ '^[a-z][a-z0-9._-]{0,63}$' THEN
    ...
    INSERT INTO app.outbox_events (
        ...
        'authz.grants_snapshot.resync_requested.tenant.v1', 1, event_payload,
...
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_projection_resync_state,
    authz.authorization_projection_resync_items TO aether_tenant_projection_worker;
