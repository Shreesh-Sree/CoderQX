...
-- Restore the pre-snapshot revision event before removing the helper that
-- materializes grants into its payload.
...
BEGIN
    ...
    INSERT INTO app.outbox_events (
        ...
        extensions.gen_random_uuid(), 'principal', p_principal_id, 'authz.revision_changed.v1', 1,
...
REVOKE EXECUTE ON FUNCTION authz.has_platform_authorization_at(uuid, bigint) FROM aether_user_app;
REVOKE EXECUTE ON FUNCTION authz.apply_authorization_snapshot(uuid, bigint, jsonb) FROM aether_user_projection_worker;
REVOKE SELECT, INSERT, UPDATE, DELETE ON authz.authorization_snapshot_inbox_messages FROM aether_user_projection_worker;
...
DROP FUNCTION users.effective_authz_grants(uuid);
...
WHERE grant_row.ctid = ranked_grant.ctid
  AND ranked_grant.position > 1;
...
ALTER TABLE authz.actor_tenant_authorizations
    ALTER COLUMN grant_source_id DROP NOT NULL;
UPDATE authz.actor_tenant_authorizations
SET grant_source_id = NULL
