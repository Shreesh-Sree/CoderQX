...
-- Restore the pre-resync helpers before removing the fail-closed state gate.
...
REVOKE EXECUTE ON FUNCTION users.process_authorization_resync_request(
...
REVOKE SELECT, INSERT, UPDATE, DELETE ON authz.authorization_projection_resync_state,
    ...
    FROM aether_user_projection_worker;
REVOKE USAGE ON SCHEMA users FROM aether_user_projection_worker;

DROP FUNCTION users.process_authorization_resync_request(uuid, bytea, timestamptz, uuid, text, text);
...
DROP TABLE users.authorization_resync_requests;
DROP TABLE users.authorization_resync_target_limits;
DROP TABLE users.authorization_resync_service_targets;
