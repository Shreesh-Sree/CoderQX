-- The migration login must be a member of aether_identity_owner.  The owner,
-- runtime, authorization-reader, and projection-worker roles are provisioned
-- before migrations by deploy/database/platform.
...
CREATE SCHEMA IF NOT EXISTS extensions AUTHORIZATION aether_identity_owner;
CREATE SCHEMA IF NOT EXISTS app AUTHORIZATION aether_identity_owner;
CREATE SCHEMA IF NOT EXISTS authz AUTHORIZATION aether_identity_owner;
CREATE SCHEMA IF NOT EXISTS identity AUTHORIZATION aether_identity_owner;
...
GRANT USAGE ON SCHEMA public TO aether_identity_migrator;

REVOKE ALL ON SCHEMA extensions, app, authz, identity FROM PUBLIC;

DO $extension_schema_access$
BEGIN
    IF has_schema_privilege('aether_identity_app', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_identity_authz_reader', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_identity_projection_worker', 'extensions', 'USAGE') THEN
        RAISE EXCEPTION 'runtime roles must not have USAGE on extensions schema';
    END IF;
END
$extension_schema_access$;

GRANT USAGE ON SCHEMA app, authz, identity TO aether_identity_app;
GRANT USAGE ON SCHEMA authz TO aether_identity_authz_reader, aether_identity_projection_worker;
...
CREATE INDEX context_keys_active_audience_idx
    ON authz.context_keys (audience, not_before, not_after)
    WHERE retired_at IS NULL;

-- Contexts are intentionally unlogged and short-lived.  A restart removes
-- them, which makes protected requests fail closed until re-authorized.
...
-- This durable replay ledger survives a PostgreSQL restart while the
-- transaction-scoped request context deliberately does not.
...
-- The projection is a current effective grant, not a source of truth.  A
-- revocation is retained as a tombstone so an out-of-order older grant cannot
-- make access live again.
...
BEGIN
    ...
    IF raw_context_id IS NULL OR raw_context_id = '' THEN
        RETURN NULL;
    END IF;
...
-- RLS must bind the signed action and resource as well as the tenant and
-- revision. A context for a read must never satisfy a write policy.
...
AS $function$
    ...
    WHERE context.context_id = app.current_context_id()
      ...
      AND context.expires_at > clock_timestamp()
      AND (
            context.tenant_id IS NULL
            OR authz.has_tenant_authorization_at(
                context.actor_id,
                context.tenant_id,
                context.authz_revision
            )
...
BEGIN
    ...
    WITH expired AS (
        SELECT context_id
        FROM authz.request_contexts
        WHERE expires_at <= clock_timestamp()
        ORDER BY expires_at
        LIMIT p_limit
        FOR UPDATE SKIP LOCKED
    ), deleted AS (
        DELETE FROM authz.request_contexts AS context
        USING expired
        WHERE context.context_id = expired.context_id
        RETURNING 1
    )
...
BEGIN
    ...
    WITH expired AS (
        ...
        FROM authz.consumed_capabilities
        WHERE expires_at <= clock_timestamp()
        ORDER BY expires_at
        LIMIT p_limit
        FOR UPDATE SKIP LOCKED
    ), deleted AS (
        DELETE FROM authz.consumed_capabilities AS capability
        USING expired
        WHERE capability.capability_id = expired.capability_id
        RETURNING 1
    )
    SELECT count(*) INTO deleted_count FROM deleted;
    RETURN deleted_count;
...
BEGIN
    ...
    SET authz_revision = EXCLUDED.authz_revision,
        ...
        grant_source_id = EXCLUDED.grant_source_id,
...
BEGIN
    IF p_actor_id IS NULL
       OR p_authz_revision <= 0
       OR p_capability_id IS NULL
       OR p_key_id IS NULL
       OR p_signature IS NULL
       OR octet_length(p_signature) <> 32
       OR p_decision <> 'allow'
    ...
    SELECT key.*
    INTO context_key
    FROM authz.context_keys AS key
    WHERE key.key_id = p_key_id
      AND key.audience = current_database()
      AND key.not_before <= v_now
      AND key.not_after > v_now
      AND key.retired_at IS NULL;

    IF NOT FOUND THEN
        RAISE EXCEPTION 'authorization context key is unavailable' USING ERRCODE = '28000';
    END IF;
    ...
    expected_signature := extensions.hmac(
        convert_to(canonical_envelope, 'UTF8'),
        context_key.key_material,
        'sha256'
    );
    ...
    IF p_tenant_id IS NOT NULL
       AND NOT authz.has_tenant_authorization_at(p_actor_id, p_tenant_id, p_authz_revision)
    THEN
...
GRANT EXECUTE ON FUNCTION app.current_context_id() TO aether_identity_app;
...
GRANT EXECUTE ON FUNCTION authz.current_context_valid(uuid) TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows(uuid, text, text) TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.current_global_context_allows(text, text) TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows_placement(uuid, text, text) TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.current_context_actor_id() TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.current_context_has_platform_access() TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.current_context_valid_placement(uuid) TO aether_identity_app;
GRANT EXECUTE ON FUNCTION authz.apply_tenant_authorization(
    uuid, uuid, bigint, boolean, text, uuid, timestamptz
) TO aether_identity_projection_worker;
GRANT SELECT ON authz.actor_tenant_authorizations TO aether_identity_authz_reader;
