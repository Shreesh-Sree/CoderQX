-- Notification stores delivery metadata and encrypted content references; app
-- sessions must carry an exact, locally projected authorization revision.
...
CREATE SCHEMA app AUTHORIZATION aether_notification_owner;
CREATE SCHEMA authz AUTHORIZATION aether_notification_owner;
CREATE SCHEMA notification AUTHORIZATION aether_notification_owner;
CREATE SCHEMA extensions AUTHORIZATION aether_notification_owner;
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;
REVOKE ALL ON SCHEMA app, authz, notification, extensions FROM PUBLIC;
DO $extension_schema_access$
BEGIN
    IF has_schema_privilege('aether_notification_app', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_notification_authz_reader', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_notification_projection_worker', 'extensions', 'USAGE') THEN
        RAISE EXCEPTION 'runtime roles must not have USAGE on extensions schema';
    END IF;
END
$extension_schema_access$;
GRANT USAGE ON SCHEMA app, authz, notification TO aether_notification_app;
GRANT USAGE ON SCHEMA authz TO aether_notification_authz_reader, aether_notification_projection_worker;

ALTER DEFAULT PRIVILEGES FOR ROLE aether_notification_owner IN SCHEMA app
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_notification_owner IN SCHEMA authz
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_notification_owner IN SCHEMA notification
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
...
CREATE TABLE app.outbox_events (
    ...
    published_at timestamptz, publish_attempts integer NOT NULL DEFAULT 0 CHECK (publish_attempts >= 0), locked_at timestamptz, lock_token uuid, last_error text,
...
CREATE TABLE app.inbox_messages (
    ...
    received_at timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP, processed_at timestamptz, attempt_count integer NOT NULL DEFAULT 0 CHECK (attempt_count >= 0), last_error text,
...
BEGIN raw_setting := current_setting('app.actor_id', true);
...
-- Signed contexts are persisted only for their five-second lifetime in an
-- unlogged table. This deliberately turns restart/projection lag into denial.
...
GRANT EXECUTE ON FUNCTION app.current_context_id() TO aether_notification_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows(uuid, text, text) TO aether_notification_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows_read(uuid, text, text, text) TO aether_notification_app;
...
GRANT SELECT ON authz.actor_tenant_authorizations TO aether_notification_authz_reader;
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.projection_inbox_messages
    TO aether_notification_projection_worker;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE app.outbox_events, app.inbox_messages, app.idempotency_keys TO aether_notification_app;
RESET ROLE;
