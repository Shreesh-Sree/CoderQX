-- Extend recipient_preferences to support email and sms delivery channels.
-- The upsert function guard is widened to permit all three channel values;
-- the table constraint is also extended to cover sms.
...
BEGIN
    ...
    IF p_preference_id IS NULL OR p_tenant_id IS NULL OR actor_id IS NULL
       OR p_channel NOT IN ('email', 'in_app', 'sms')
       OR p_enabled IS NULL OR p_expected_version IS NULL
