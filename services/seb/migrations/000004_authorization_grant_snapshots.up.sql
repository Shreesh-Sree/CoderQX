-- Replace the one-row-per-tenant legacy projection with complete canonical
-- grant snapshots. A revision tombstone remains even for an empty grant set,
-- so a revocation cannot be mistaken for a lagging but still-valid grant.
...
CREATE TABLE authz.principal_authorization_revisions (
	...
	-- Legacy single-grant rows are intentionally not trusted until a complete
	-- snapshot has arrived for the same revision.
...
-- Retain the legacy event function for a controlled transition. Normal
-- operation uses apply_authorization_snapshot below; a stale compatibility
-- event can never overwrite a newer complete snapshot.
...
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_snapshot_inbox_messages
    TO aether_seb_projection_worker;
GRANT SELECT ON authz.principal_authorization_revisions TO aether_seb_authz_reader;
