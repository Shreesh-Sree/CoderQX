-- Assessment owns tenant-scoped exam snapshots and assignments. The app role is
-- deliberately unable to inspect or mutate event-fed authorization projections.
...
BEGIN
    FOREACH expected_role IN ARRAY ARRAY[
        'aether_assessment_owner',
        'aether_assessment_migrator',
        'aether_assessment_app',
        'aether_assessment_authz_reader',
        'aether_assessment_projection_worker'
    ]
    LOOP
        ...
        IF EXISTS (
            ...
            WHERE rolname = expected_role AND (rolsuper OR rolbypassrls)
        ) THEN
...
CREATE SCHEMA app AUTHORIZATION aether_assessment_owner;
CREATE SCHEMA authz AUTHORIZATION aether_assessment_owner;
CREATE SCHEMA assessment AUTHORIZATION aether_assessment_owner;
CREATE SCHEMA extensions AUTHORIZATION aether_assessment_owner;

REVOKE ALL ON SCHEMA extensions FROM PUBLIC;
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;
DO $extension_schema_access$
BEGIN
    IF has_schema_privilege('aether_assessment_app', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_assessment_authz_reader', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_assessment_projection_worker', 'extensions', 'USAGE') THEN
        RAISE EXCEPTION 'runtime roles must not have USAGE on extensions schema';
    END IF;
END
$extension_schema_access$;

REVOKE ALL ON SCHEMA app, authz, assessment FROM PUBLIC;
GRANT USAGE ON SCHEMA app, authz, assessment TO aether_assessment_app;
GRANT USAGE ON SCHEMA authz TO aether_assessment_authz_reader, aether_assessment_projection_worker;

ALTER DEFAULT PRIVILEGES FOR ROLE aether_assessment_owner IN SCHEMA app
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_assessment_owner IN SCHEMA authz
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_assessment_owner IN SCHEMA assessment
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
...
CREATE INDEX projection_inbox_messages_pending_idx ON authz.projection_inbox_messages (received_at) WHERE processed_at IS NULL;

CREATE FUNCTION authz.context_signature_payload(actor uuid, tenant uuid, revision bigint, decision uuid, issued_epoch bigint, expires_epoch bigint, key uuid)
RETURNS bytea LANGUAGE sql STABLE SET search_path = pg_catalog AS $$
    SELECT convert_to(format('aethercode-authz-context-v1|%s|%s|%s|%s|%s|%s|%s|%s',
        current_database(), actor::text, COALESCE(tenant::text, ''), revision::text,
        decision::text, issued_epoch::text, expires_epoch::text, key::text), 'UTF8');
$$;

CREATE FUNCTION authz.set_context(actor uuid, tenant uuid, revision bigint, decision uuid, issued_epoch bigint, expires_epoch bigint, key uuid, signature bytea)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, app, authz AS $$
DECLARE signing_secret bytea; issued_at_value timestamptz; expires_at_value timestamptz;
BEGIN
    ...
    IF expires_at_value <= issued_at_value OR expires_at_value > issued_at_value + interval '60 seconds'
       OR issued_at_value > CURRENT_TIMESTAMP + interval '30 seconds' OR expires_at_value <= CURRENT_TIMESTAMP THEN
        RAISE EXCEPTION 'authorization context is outside its allowed lifetime' USING ERRCODE = '28000';
    END IF;
    SELECT context_key.hmac_secret INTO signing_secret FROM authz.context_keys context_key
    WHERE context_key.id = key AND context_key.valid_from <= CURRENT_TIMESTAMP AND context_key.valid_until > CURRENT_TIMESTAMP
      AND (context_key.retired_at IS NULL OR context_key.retired_at > CURRENT_TIMESTAMP);
    IF signing_secret IS NULL OR signature <> hmac(authz.context_signature_payload(actor, tenant, revision, decision, issued_epoch, expires_epoch, key), signing_secret, 'sha256') THEN
        RAISE EXCEPTION 'authorization context signature is invalid' USING ERRCODE = '28000';
    END IF;
    INSERT INTO authz.request_contexts (backend_pid, transaction_id, actor_id, tenant_id, authz_revision, decision_id, key_id, issued_at, expires_at)
    VALUES (pg_backend_pid(), txid_current(), actor, tenant, revision, decision, key, issued_at_value, expires_at_value)
    ON CONFLICT (backend_pid, transaction_id) DO UPDATE SET
        actor_id = EXCLUDED.actor_id, tenant_id = EXCLUDED.tenant_id, authz_revision = EXCLUDED.authz_revision,
        decision_id = EXCLUDED.decision_id, key_id = EXCLUDED.key_id, issued_at = EXCLUDED.issued_at,
        expires_at = EXCLUDED.expires_at, validated_at = CURRENT_TIMESTAMP;
...
CREATE FUNCTION authz.has_valid_context() RETURNS boolean LANGUAGE sql STABLE SECURITY DEFINER
SET search_path = pg_catalog, app, authz AS $$
    SELECT EXISTS (
        SELECT 1 FROM authz.request_contexts context
...
CREATE INDEX actor_tenant_authorizations_tenant_idx
    ON authz.actor_tenant_authorizations (tenant_id, actor_id)
    WHERE active;

CREATE FUNCTION authz.has_tenant_access(requested_tenant_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
    SELECT authz.has_valid_context()
       AND requested_tenant_id IS NOT NULL
       AND requested_tenant_id = app.current_tenant_id()
       AND EXISTS (
            SELECT 1
            FROM authz.actor_tenant_authorizations authorization_row
            WHERE authorization_row.actor_id = app.current_actor_id()
              AND authorization_row.tenant_id = requested_tenant_id
...
-- Keep the final signed-context representation in the initial bootstrap. This
-- avoids a released destructive migration that could discard authorization
-- keys or leave a rollback with broken RLS functions.
...
GRANT EXECUTE ON FUNCTION app.current_context_id() TO aether_assessment_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows(uuid, text, text) TO aether_assessment_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows_read(uuid, text, text, text) TO aether_assessment_app;
...
GRANT SELECT ON authz.actor_tenant_authorizations TO aether_assessment_authz_reader;
