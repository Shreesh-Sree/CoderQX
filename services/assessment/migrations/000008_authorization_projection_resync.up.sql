-- A durable full snapshot is required after a consumer is offline beyond
-- JetStream retention. Until the matching manifest is complete, the local
-- authorization projection is deliberately unusable by FORCE RLS.
...
RETURNS boolean LANGUAGE sql VOLATILE SECURITY DEFINER SET search_path = pg_catalog, authz AS $function$
    SELECT authz.authorization_projection_ready()
       AND p_tenant_id IS NOT NULL AND EXISTS (
        SELECT 1
...
BEGIN
    ...
    SET active_resync_id = p_resync_id, request_event_id = p_request_event_id,
        projection_ready = false, requested_at = event_occurred_at,
        completion_event_id = NULL, expected_snapshot_count = NULL,
        expected_manifest_sha256 = NULL, completed_at = NULL, last_error = NULL,
        updated_at = event_occurred_at
    WHERE singleton = true;

    event_payload := jsonb_build_object(
        'resync_id', p_resync_id, 'target_service', p_target_service, 'reason', p_reason
    );
    INSERT INTO app.outbox_events (
        ...
        'authz.grants_snapshot.resync_requested.assessment.v1', 1, event_payload,
...
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_projection_resync_state,
    authz.authorization_projection_resync_items TO aether_assessment_projection_worker;
