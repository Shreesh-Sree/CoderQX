-- Complete User grant snapshots replace the bootstrap one-row tenant grant.
-- Until one complete snapshot is applied, RLS fails closed.
...
-- Retained only for controlled compatibility during rollout. It can create a
-- revision tombstone but can never be trusted until a complete snapshot lands.
...
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_snapshot_inbox_messages
    TO aether_analytics_projection_worker;
GRANT SELECT ON authz.principal_authorization_revisions TO aether_analytics_authz_reader;
