-- Analytics read models are RLS-protected. A retained grant projection is
-- never trusted after a snapshot-consumer recovery gap; a targeted complete
-- batch must be reconciled before requests can establish local context.
...
BEGIN
    IF p_request_event_id IS NULL OR p_resync_id IS NULL
       ...
       OR p_target_service <> 'analytics' OR p_reason !~ '^[a-z][a-z0-9._-]{0,63}$' THEN
    ...
    INSERT INTO app.outbox_events (
        ...
        'authz.grants_snapshot.resync_requested.analytics.v1', 1, event_payload,
...
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.authorization_projection_resync_state,
    authz.authorization_projection_resync_items TO aether_analytics_projection_worker;
