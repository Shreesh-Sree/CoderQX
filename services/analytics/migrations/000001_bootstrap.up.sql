-- Analytics is an event-fed read-model database. It receives only versioned
-- events, exposes tenant-scoped views, and never directly joins operational DBs.
...
BEGIN
    FOREACH expected_role IN ARRAY ARRAY[
        'aether_analytics_owner',
        'aether_analytics_migrator',
        'aether_analytics_app',
        'aether_analytics_authz_reader',
        'aether_analytics_projection_worker'
    ] LOOP
...
CREATE SCHEMA app AUTHORIZATION aether_analytics_owner;
CREATE SCHEMA authz AUTHORIZATION aether_analytics_owner;
CREATE SCHEMA analytics AUTHORIZATION aether_analytics_owner;
CREATE SCHEMA extensions AUTHORIZATION aether_analytics_owner;
CREATE EXTENSION IF NOT EXISTS pgcrypto WITH SCHEMA extensions;
REVOKE ALL ON SCHEMA app, authz, analytics, extensions FROM PUBLIC;
DO $extension_schema_access$
BEGIN
    IF has_schema_privilege('aether_analytics_app', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_analytics_authz_reader', 'extensions', 'USAGE')
       OR has_schema_privilege('aether_analytics_projection_worker', 'extensions', 'USAGE') THEN
        RAISE EXCEPTION 'runtime roles must not have USAGE on extensions schema';
    END IF;
END
$extension_schema_access$;
GRANT USAGE ON SCHEMA app, authz, analytics TO aether_analytics_app;
GRANT USAGE ON SCHEMA authz TO aether_analytics_authz_reader, aether_analytics_projection_worker;

ALTER DEFAULT PRIVILEGES FOR ROLE aether_analytics_owner IN SCHEMA app
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_analytics_owner IN SCHEMA authz
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
ALTER DEFAULT PRIVILEGES FOR ROLE aether_analytics_owner IN SCHEMA analytics
    REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC;
...
-- Contexts are unlogged by design. Restarting the database removes them and
-- fails protected work closed until central authorization is re-established.
...
GRANT EXECUTE ON FUNCTION app.current_context_id() TO aether_analytics_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows(uuid, text, text) TO aether_analytics_app;
GRANT EXECUTE ON FUNCTION authz.current_context_allows_read(uuid, text, text, text) TO aether_analytics_app;
...
GRANT SELECT ON authz.actor_tenant_authorizations TO aether_analytics_authz_reader;
GRANT SELECT, INSERT, UPDATE, DELETE ON authz.projection_inbox_messages
    TO aether_analytics_projection_worker;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE app.outbox_events, app.inbox_messages, app.idempotency_keys TO aether_analytics_app;
RESET ROLE;
